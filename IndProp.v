...
Proof.
  ...
  - (* n = 1 *) simpl. intros _.  apply ev_0.
  - (* n = n' + 2; we're stuck! *) simpl. Abort.
...
Proof.
  intros n m En Em. induction En as [| In' En' IHEn'].
  - simpl. apply Em.
...
Proof.
  intros n m p Hnm Hnp. apply ev_ev__ev with (n + n). replace (n + n + (m + p)) with ((n + m) + (n + p)).
  - apply ev_sum.
    + apply Hnm.
    + apply Hnp.
  - SearchAbout plus_comm. SearchAbout plus. rewrite plus_swap at 1. rewrite <- plus_assoc. rewrite plus_assoc. reflexivity.
...
Theorem lt_S : forall n m,
  ...
  n < S m.
Proof.
  intros. unfold lt. apply le_S in H. apply H.
Qed.
...
Proof.
  ...
  - destruct m.
    + intro. apply le_n.
    + intro. apply le_S. apply O_le_n.
  - destruct m.
    + intro. inversion H.
    + intro. simpl.  apply n_le_m__Sn_le_Sm. apply IHn. simpl in H. apply H.
Qed.
...
Proof.
  ...
  - destruct n. 
    + intro. simpl. reflexivity.
    + intro. simpl. apply IHm. apply Sn_le_Sm__n_le_m in H. apply H.
Qed.
...
Proof.
  intros n m o Hnm Hmo. apply leb_correct. apply le_trans with (n := m). 
  - apply leb_complete. apply Hnm.
  - apply leb_complete. apply Hmo.
Qed.
...
Proof.
  intros. split.
  - intro. apply leb_complete. apply H.
  - intro. apply leb_correct. apply H.
Qed.
(** [] *)
...
(** - Which of the following propositions are provable?
      - [R 1 1 2] match with c4 -> c1.
      - [R 2 2 6] match c5

    - If we dropped constructor [c5] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer.
      - Oui R 2 2 6 n'est plus prouvable.

    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer.
      - Oui R 1 1 2 n'est plus prouvable.
...
Proof.
  ...
  - intro. induction H.
    ...
    + simpl. unfold fR. rewrite <- plus_n_Sm. unfold fR in IHR. rewrite IHR. reflexivity.
    + inversion IHR. unfold fR in H1. rewrite <- plus_n_Sm in H1. inversion H1. unfold fR. reflexivity.
    + unfold fR in IHR. unfold fR. rewrite plus_comm. apply IHR.
  - intro. destruct H. induction m.
    + simpl. induction n.
      *  apply c1.
      *  apply c3. apply IHn.
    + simpl. induction n.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | finish_case l : subseq nil l
  | not_case n l1 l2 : subseq l1 l2 -> subseq l1 (n :: l2)
  | equal_case n l1 l2 : subseq l1 l2 -> subseq (n :: l1) (n :: l2).

Notation "[ ]" := nil.
Notation "[ x ; .. ; y ]" := (cons x .. (cons y nil) ..).

Theorem test_subseq_exemple : subseq [1;2;3] [5;6;1;9;9;2;7;3;8].
Proof.
  apply not_case, not_case.
  apply equal_case. (* found 1. *)
  apply not_case, not_case.
  apply equal_case. (* found 2. *)
  apply not_case.
  apply equal_case. (* found 3. *)
  apply finish_case.
Qed. (* ca a l'air de marcher ! *)

Theorem subseq_refl : forall l, subseq l l.
Proof.
  intro. induction l.
  - apply finish_case.
  - apply equal_case. apply IHl.
Qed.
...
Proof.
  intros. induction H.
  - apply finish_case.
  - simpl. apply not_case. apply IHsubseq.
  - simpl. apply equal_case. apply IHsubseq.
Qed.

Theorem subseq_trans : forall l1 l2 l3,
    subseq l1 l2 -> subseq l2 l3 -> subseq l1 l3.
Proof.
  intros. generalize dependent H. generalize dependent l1. induction H0.
  - intros. inversion H. apply finish_case.
  - intros l3 H. apply not_case. apply IHsubseq. apply H.
  - intros l3 H. inversion H.
    + apply finish_case.
    + apply not_case. apply IHsubseq. apply H3.
    + apply equal_case. apply IHsubseq. apply H3.
Qed.
(** [] *)
...
(** Suppose we give Coq the following definition:
    ...
    - [R 2 [1;0]] this one. -> c2 = R 1 [0] -> c2 - R 0 [].
    - [R 1 [1;2;1;0]] this one -> c3 (R2) reduit 1. -> R 1 2 : c3 c3 qui donne R3 2 reduit 2 -> R2 1 reduit 1. R1 0 reduit 0, puis R0 [].
    - [R 6 [3;2;1;0]] not this one -> R6 ne peut jamais reduire 3, 6 n'est jamais S (n) et on ne peux que l'incrementer. *) 
...
Proof.
  intros. split.
  - intro. generalize dependent s1. induction s2.
    + intro. intro H. inversion H. reflexivity.
    + intros s1 H. inversion H. inversion H3. simpl. apply IHs2 in H4. rewrite H4. reflexivity.
  - intro. generalize dependent s1. induction s2.
...
Fixpoint re_not_empty {T} (re : reg_exp T) : bool :=
 match re with
  ...
  | EmptyStr => true
  | Char x => true
  | App re1 re2 => orb (re_not_empty re1)  (re_not_empty re2)
...
Proof.
  ...
  - intros. induction re.
    ...
    + simpl. reflexivity.
    + simpl. reflexivity.
    + simpl. apply orb_true_iff. left. apply IHre1. inversion H. inversion H0. exists s1. apply H4.
    + simpl. apply orb_true_iff. Print MUnion'. constructor. apply IHre1. inversion H. Print MUnionL. exists x. (* A FINIR *)
Admitted.
(** [] *)
...
Proof.
  ...
  - (* MEmpty *)
    simpl. omega.
  - simpl. omega.
  - simpl. intros Hlen. assert(H: pumping_constant re1 <= length s1 \/ pumping_constant re2 <= length s2).
    { rewrite app_length in Hlen. omega. }
    destruct H as [H | H ].
    + destruct (IH1 H) as [s11 [s12 [s13 [H1 [H2 H3]]]]]. rewrite H1. exists s11. exists s12.  exists (s13 ++ s2). rewrite <- app_assoc. rewrite <- app_assoc. split.
      *  reflexivity.
      *  split.
        --  apply H2.
        --  
...
Proof.
  ...
  - split.
    + intro. reflexivity.
    + intro. inversion H. apply H1.
  - split.
    + intro. inversion H. exfalso. apply (H1 H0).
    + intro. inversion H0.
Qed.
(** [] *)
