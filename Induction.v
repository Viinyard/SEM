...
(** One inconveninent aspect of our definition of [evenb n] is the
    ...
    induction hypothesis about [n - 2]. The following lemma gives a
    better characterization of [evenb (S n)]: *)
...
(* On utilise destruct pour prouver un theoreme avec un nombre fini de cas, tandis que l'induction
est utilise pour tout cas, mais sur une infinite de cas. Destruct permet de traiter tout les cas n d'un match n with*)
*)
(** [] *)
...
Proof.
  intros n m.
  assert (H: 0 + n = n). { reflexivity. } (** nom: la propriete -> il faut que tu prouve H ensuite on fait la suite **)
  rewrite -> H.
  reflexivity.  Qed.
...
Proof.
  ...
  assert (A: m + n = n + m).  { rewrite -> plus_comm. reflexivity. }
  rewrite -> A. reflexivity. Qed.

(** Now prove commutativity of multiplication.  (You will probably
    need to define and prove a separate subsidiary theorem to be used
    in the proof of this one.  You may find that [plus_swap] comes in
    handy.) *)
Lemma mult_m_1 : forall n : nat,
  n * 1 = n.
Proof.
  intros n. induction n as [| n' IHn'].
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl.  rewrite -> mult_S. rewrite -> IHn'. rewrite <- plus_comm. reflexivity.
Qed.

Lemma mult_n_0 : forall n : nat,
  n * 0 = 0.
Proof.
  intros n. rewrite <- mult_comm. simpl. reflexivity.
Qed.
...
Lemma leb_sep : forall n m p : nat,
  leb (p + n) (p + m) = andb (leb p p) (leb n m).
Proof.
...
Proof.
  ...
  - simpl. rewrite -> leb_sep. rewrite <- leb_refl. simpl. intros H. rewrite -> H. reflexivity.
Qed.
...
Proof.
  intros n.
  rewrite -> mult_comm. rewrite -> mult_m_1. reflexivity.
Qed.
...
Lemma mult_add_comm : forall n m p,
  n + (m * p + m) = m * p + (n + m).
Proof.
  intros n m p. rewrite -> plus_swap. reflexivity.
Qed.

Theorem mult_plus_distr_r_first_try : forall n m p : nat,
  (n + m) * p = (n * p) + (m * p).
Proof.
  intros n m p. induction p as [| p' IHp'].
  - simpl. rewrite -> mult_n_0. rewrite -> mult_n_0. rewrite -> mult_n_0. reflexivity.
  - rewrite -> mult_S.  rewrite -> mult_S. rewrite -> mult_S. rewrite -> IHp'. rewrite -> mult_add_comm.
Abort.
...
Theorem plus_swap'_f : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros n m p. induction n as [| n' IHn'].
  - simpl. reflexivity.
...
Theorem binary_commute : forall  b:bin,
  convert(b) + 1 = (convert(incr b)).
Proof.
  intros b . induction b as [| b' IHb' | b'' IHb'']  .
  - simpl. reflexivity.
  - simpl. rewrite <- plus_n_O. reflexivity.
  - simpl. rewrite <- IHb''. rewrite -> plus_assoc. rewrite -> plus_assoc. rewrite -> plus_assoc. rewrite <- plus_n_O. rewrite <- plus_n_O.
    rewrite -> plus_comm with (n:= convert b'' + 1) (m := convert b'').
    rewrite -> plus_assoc. reflexivity.
Qed.
...
For any [n] and [m],
    ...
    - Next, suppose [n = S n'], where
      ...
      Par la reciproque de  [plus_n_Sm], this follows from

        S (n' + m ) = m + S n'.
      On suppose que n + m = m + n. alors :
