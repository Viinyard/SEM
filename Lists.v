...
Proof.
  intros s. induction s as [| s' s'' IHs'].
  - simpl. reflexivity.
  - simpl. destruct s'.
...
Proof.
  intros s1 s2 v. induction s1 as [| n s1' IHn'].
  - simpl. reflexivity.
  - simpl. destruct (beq_nat v n).
...
There is just two element of type baz, and there is Baz1 and Baz2. It's impossible to create a value of type baz.
Because Baz1 has the type baz -> baz, so he need a value of type base to be created.
And Baz2 has the type baz -> bool -> baz. So he need a value of type base (and a value of type bool) to be created too.
There is a self dependencie between this two constructors. Baz1 and Baz2 need to be initiated by an already existed value of type baz.
*)
(** [] *)
