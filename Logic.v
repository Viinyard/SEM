...
Theorem or_commut : forall P Q : Prop,
  P \/ Q  -> Q \/ P.
Proof.
  intros. destruct H. right. apply H. left. apply H. Qed.
(** [] *)
...
Theorem contrapositive : forall P Q : Prop,
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q HP HQ. unfold not in HQ. unfold not. intros HP'. apply HP in HP'. apply HQ in HP'. apply HP'.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intro H. destruct H. apply H0. apply H. Qed.
(** [] *)
...
Theorem iff_refl : forall P : Prop,
  P <-> P.
Proof.
  intros P. unfold iff. split. intros H. apply H. intros H. apply H. Qed.

Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros. unfold iff in H. destruct H. unfold iff in H0. destruct H0. unfold iff. split. 
...
Proof.
  ...
  - intro. inversion H as [ HP | [ HQ HR ] ].
    ...
    + split.
      * right. apply HQ.
      * right. apply HR.
  - intro H. inversion H as [ [HP | HQ] [HP' | HR']].
    + left. apply HP.
    + left. apply HP.
    + left. apply HP'.
    + right. split.
...
Proof.
  intros. unfold not. intro. inversion H0. apply H1. apply H.
Qed.
(** [] *)
...
Proof.
   intros. split.
   - intro. inversion H as [x [HEP | HEQ]].
    + left. exists x. apply HEP.
    + right. exists x. apply HEQ.
  - intro. inversion H as [[x HEP] | [x HEQ]].
    + exists x. left. apply HEP.
    + exists x. right. apply HEQ.
Qed.
(** [] *)
...
Lemma In_true : forall (A : Type) (x y : A) (l : list A),
  In x l -> In x (y::l).
Proof.
  intros. induction l.
  - simpl. inversion H.
  ...
  - intro. induction l. 
    ...
    + simpl. simpl in H. destruct H. 
      * exists x. split. 
        { apply H. } 
        { left. reflexivity. }
      * destruct IHl. apply H. exists x0. destruct H0. split. { apply H0. } { right. apply H1. }
  - intro. inversion H as [x [Hy Hx]]. induction l.
    + inversion Hx.
    + rewrite <- Hy. apply In_map. apply Hx. (* 2H sur cette ligne !! *)
Qed. 
(** [] *)
...
Proof.
  ...
  - intro. induction l.
    ...
    + simpl in H. simpl. inversion H. 
      * rewrite H0. left. left. reflexivity.
      * destruct IHl. 
          { apply H0. }
          { left. right. apply H1. }
          { right. apply H1. }
...
Proof.
  ...
  - induction l.
    ...
    + simpl. split. 
      ...
      * apply IHl. intro. intro. apply H. right. apply H0.
  - induction l.
    + intros. inversion H0.
    + intros. simpl in H. simpl in H0. inversion H. inversion H0.
...
Proof.
  ...
  - intro. simpl. rewrite IHl1. symmetry. rewrite IHl1. rewrite <- app_assoc. simpl. reflexivity.
Qed.


Lemma tr_rev_correct : forall X, @tr_rev X = @rev X.
Proof.
  intro. apply functional_extensionality. unfold tr_rev. induction x. 
  - simpl. reflexivity.
  - simpl. rewrite <- IHx. apply tr_rev_ref.
Qed.
(** [] *)
...
Proof.
  ...
  - Print evenb_S. rewrite -> evenb_S. destruct (evenb n).
...
Proof.
 intros. split. 
 - destruct (beq_nat x y) eqn: h1.
    + intro. inversion H.
    + intros. intro. rewrite H0 in h1. rewrite <- beq_nat_refl in h1. inversion h1.
 - destruct (beq_nat x y) eqn: h1.
    + intro. destruct H. apply beq_nat_true. apply h1.
    + intro. reflexivity.
Qed.
(** [] *)
...
Fixpoint beq_list {A} (beq : A -> A -> bool)
  ...
  | a1::l1' => match l2 with
      | nil => false
      | a2::l2' => (andb (beq a1 a2) (beq_list beq l1' l2'))
      end
  end.
