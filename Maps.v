...
Proof.
  intros. unfold t_update. rewrite false_beq_id. 
  - reflexivity.
  - apply H.
Qed.
(** [] *)
...
Lemma t_update_shadow : forall A (m: total_map A) v1 v2 x,
  ...
  = t_update m x v2.
Proof.
  intros. unfold t_update. extensionality x'. destruct (beq_id x x'); reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. destruct (beq_id x y) eqn: H. 
  - SearchAbout reflect. apply ReflectT. apply beq_id_true_iff. apply H.
...
Theorem t_update_same : forall X x (m : total_map X),
  t_update m x (m x) = m.
Proof.
  intros. unfold t_update. extensionality x'. SearchAbout reflect. destruct (beq_id x x') eqn: H. 
...
Proof.
  intros. unfold t_update. extensionality x'. destruct (beq_id x1 x') eqn: H'.
  - apply beq_id_true_iff in H'. rewrite H' in H. rewrite false_beq_id.
    + reflexivity.
    + apply H.
  - destruct (beq_id x2 x'); repeat (reflexivity).
Qed.
(** [] *)
