...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?
      - [d (b a 5)] => FAUX (il manque un type X)
      - [d mumble (b a 5)] => VRAI (comme avant mais typé)
      - [d bool (b a 5)] => VRAI (idem, tout les types sont accepté)
      - [e bool true] => VRAI
      - [e mumble (b c 0)] => VRAI
      - [e bool (b c 0)] => Faux; constructeur de mumble n'attend pas de type
      - [c] => Vrai
(* FILL IN HERE *)
*)
(** [] *)
...
Theorem rev_app : forall X (l1 l2 : list X),
  rev (l1++l2) = rev(l2) ++ rev(l1).
Proof.
...
Proof.
  intros X l. induction l as [| n l' IHl].
  - simpl. reflexivity.
  - simpl. rewrite -> rev_app. rewrite -> IHl. simpl. reflexivity.
Qed.
(** [] *)
...
Example ex1 : leb 7 7  = true.
Proof. simpl. reflexivity. Qed.

Example ex2 : leb 9 7  = false.
Proof. simpl. reflexivity. Qed.
...
Theorem map_f_comm : forall (X Y : Type) (f : X -> Y) (x : X) (l : list X),
  map f (l ++ [x]) = map f l ++ [f x].
Proof.
  intros X Y f x l. induction l.
  - simpl. reflexivity.
  - simpl. rewrite -> IHl. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHl. rewrite -> map_f_comm. rewrite -> IHl. reflexivity.
Qed.
(** [] *)
...
Theorem length_comm : forall X (l : list X) (x0 : X),
  length (x0::l) = S(length l).
Proof.
...
Proof.
  intros X Y f l. induction l.
  - reflexivity. 
  - simpl. unfold fold_map. simpl. unfold fold_map in IHl. rewrite IHl. reflexivity.
Qed.
(** [] *)
...
Definition mult (n m : nat) : nat :=
  fun (X : Type) (f : X -> X) => m X (n X f). (* pas trop compris pourquoi il veut pas de f x *)
...
(** $Date: 2016-07-12 03:31:32 +0200 (Mar, 12 jul 2016) $ *)
