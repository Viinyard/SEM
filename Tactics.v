...
(* Rewrite a un sens, gauche ou droite et permet de reecrire une partie de l'equation par un theoreme deja prouve, ou une hypothese pour l'induction
    Apply permet d'appliquer automatiquement un theorem deja prouve sur les deux partie de l'équation. Quand une equation a la forme de quelque chose de deja prouve precedement.
*) *)
(** [] *)
...
Proof.
  intros.
  - apply trans_eq with (minustwo o). rewrite -> H0. rewrite -> H. reflexivity. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros m H. simpl in H. induction m.
    +  reflexivity.
    + inversion H.
  - intros. simpl in H. induction m.
    + inversion H.
    + rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H. inversion H. apply IHn' in H1. rewrite -> H1. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. intros. generalize dependent n. destruct n.
    +  intro H. inversion H.
    + intro H. inversion H. rewrite H1. apply f_equal. apply IHl1. apply H1.
Qed.
(** [] *)

(** **** Exercise: 4 stars, optional (app_length_twice)  *)
(** Prove this by induction on [l], without using [app_length] from [Lists]. *)
Theorem app_length_cons_invective : forall (X : Type) (l1 l2 : list X)
                                  (x : X) (n : nat),
...
Proof.
  ...
  -   simpl. intros. destruct n.
    + simpl. inversion H.
    + simpl. inversion H. apply IHl in H1. apply f_equal with (f:= S) in H1. apply app_length_cons_invective with (x:=x) in H1. rewrite H1. simpl. inversion H. rewrite plus_n_Sm. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros n. induction n. apply H. apply H1. apply IHn.
  - intros n. induction n. apply H0. apply IHm. apply H2. apply IHm.
Qed.
(** [] *)
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  intros. generalize dependent l2. generalize dependent l1. induction l.
  - simpl. intros. inversion H. simpl. reflexivity.
  - intros l1 l2. intros H. rewrite IHl. inversion H. 
Abort. (* REVENIR ICI*)
(** [] *)
...
Proof.
 intros. destruct b.
  - destruct (f true) eqn:Heqe1.
    + destruct (f true) eqn:Heqe2.
      * apply Heqe2.
      * destruct (f false) eqn:Heqef1. reflexivity. apply Heqe1.
    + destruct (f false) eqn:Heqef2. apply Heqe1. apply Heqef2.
  - destruct (f false) eqn:f1.
    +  destruct (f true) eqn:t1.
      * apply t1.
      * apply f1.
    + destruct (f false) eqn:f2.
      * destruct (f true) eqn:t1. apply f1. reflexivity.
      * apply f2.
Qed.
(** [] *)
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat n p = true.
Proof.
  intros. apply beq_nat_true in H. apply beq_nat_true in H0. rewrite -> H. rewrite <- H0. symmetry. apply beq_nat_refl.
Qed.
(** [] *)
...
Proof.
unfold split_combine_statement. intros.
  - induction l1.
